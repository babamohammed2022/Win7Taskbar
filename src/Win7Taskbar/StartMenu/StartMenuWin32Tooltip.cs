    ...
    internal sealed class StartMenuWin32Tooltip : IDisposable
    {
        ...
        public void Show(IntPtr owner, string? title, string? text, int screenX, int screenY)
        {
            ...
            try
            {
                ...
                /* TTM_TRACKPOSITION is the pixel the tip is shown at
                 * (TTF_ABSOLUTE). Microsoft's tracking-tooltip sample
                 * offsets that point so the window does not sit on the
                 * mouse pointer (learn.microsoft.com: How to Implement
                 * Tracking Tooltips — pt.x + 10, pt.y - 20). 6 px is
                 * the same idea at the distance asked for. */
                SendMessage(_hwnd, TTM_TRACKPOSITION, IntPtr.Zero,
                    PackPoint(screenX + 6, screenY + 6));
