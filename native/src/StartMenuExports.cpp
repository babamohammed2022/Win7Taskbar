...
wchar_t FileKindFromExtension(const wchar_t* name) {
    ...
    wchar_t ext[12] = {};
    std::size_t n = 0;
    for (const wchar_t* p = dot + 1; *p && n < _countof(ext) - 1; ++p) {
        ext[n++] = static_cast<wchar_t>(towlower(*p));
    }
    struct Family { wchar_t kind; const wchar_t* exts; };
    /* Ogni voce comincia e finisce con spazi: il confronto " ext " lavora
     * sull'ultimo elemento della lista come su tutti gli altri. */
    static const Family kFamilies[] = {
        { L'D', L" doc docx txt rtf odt pdf xps csv xls xlsx ods ppt pptx odp"
                L" epub mobi md log ini htm html xml json " },
        { L'P', L" jpg jpeg png gif bmp ico tif tiff webp heic raw cr2 nef"
                L" dng svg " },
        { L'M', L" mp3 wav flac ogg wma m4a aac mid midi opus mp2 aif aiff " },
        { L'V', L" mp4 mkv wmv avi mov mpg mpeg webm m2ts vob flv ts 3gp " },
    };
    for (const Family& fam : kFamilies) {
        const wchar_t* f = fam.exts;
        const std::wstring probe = std::wstring(L" ") + ext + L" ";
        if (wcsstr(f, probe.c_str()) != nullptr) {
            return fam.kind;
        }
    }
    return L'F';
...
void WalkFilesForQuery(const std::wstring& folder, const std::wstring& query,
    ...
    do {
        ...
        const wchar_t kind = FileKindFromExtension(name);
        std::wstring line;
...
void FileSearchWorker(std::wstring query, uint32_t generation) {
    ...
    try {
        ...
        if (!roots.empty()) {
            ...
            /* Ordinamento stabile per famiglia D -> P -> M -> V -> F,
            ...
            auto kindRank = [](const std::wstring& line) {
                static const wchar_t kOrder[] = L"DPMVF";
