...
const ULONG_PTR kCopyDataIconRect = 3;

/* Registra una classe privata e ricorda se questa istanza del processo ne e'
 * proprietaria. ERROR_CLASS_ALREADY_EXISTS e' accettato solo quando la
 * classe gia' presente appartiene allo stesso modulo e usa la stessa WndProc:
 * non si usurpa una classe locale di un altro componente. */
bool RegisterPrivateWindowClass(const WNDCLASSEXW& windowClass,
                                bool& owned) noexcept {
    owned = false;
    ...
    if (GetClassInfoExW(windowClass.hInstance,
                        windowClass.lpszClassName, &existing) == FALSE) {
        return false;
    }
    return existing.hInstance == windowClass.hInstance &&
           existing.lpfnWndProc == windowClass.lpfnWndProc;
}

void UnregisterPrivateWindowClass(const wchar_t* className,
                                  HINSTANCE instance,
                                  bool& owned) noexcept {
    if (owned && className != nullptr && instance != nullptr) {
...
bool TrayService::CreateWindows() {
    ...
    /* Se una registrazione o una finestra successiva fallisce, il guardiano
     * distrugge cio' che e' gia' stato creato e annulla le classi possedute. */
    auto rollback = MakeScopeExit([this] { DestroyWindows(); });
    ...
    const bool switchClassReady =
        RegisterPrivateWindowClass(switchWc, m_taskSwitchClassOwned);
    if (switchClassReady) {
    ...
    if (SetTimer(m_trayWnd, kTimerTrayMonitor, 100, nullptr) == 0) {
        /* E' una protezione di z-order: se il timer non parte, il flag
         * WS_EX_TOPMOST continua comunque a lasciare operativo il ricevitore. */
...
void TrayService::DestroyWindows() {
    ...
    /* Le classi sono private al processo: si annullano in ordine inverso
     * dopo avere distrutto tutte le finestre che le usano. */
    UnregisterPrivateWindowClass(L"W7T_TaskSwitch", m_windowInstance,
                                 m_taskSwitchClassOwned);
    UnregisterPrivateWindowClass(L"TrayNotifyWnd", m_windowInstance,
                                 m_notifyClassOwned);
    UnregisterPrivateWindowClass(L"Shell_TrayWnd", m_windowInstance,
                                 m_trayClassOwned);
...
void TrayService::ThreadMain() {
    ...
    try {
        windowsCreated = CreateWindows();
    } catch (...) {
        AppendCoreLog(L"tray: eccezione durante la creazione delle finestre");
...
LRESULT TrayService::HandleCopyData(HWND hwnd, WPARAM sender,
    ...
    W7T_SEH_TRY {
        try {
            /* I pacchetti AppBar, Shell_NotifyIcon e GetRect che non
             * vengono gestiti localmente devono continuare verso Explorer.
             * Un risultato TRUE locale non viene inventato per mascherare un
             * pacchetto malformato. */
            shouldForward = cds != nullptr &&
                (cds->dwData == kCopyDataAppBar ||
                 cds->dwData == kCopyDataTrayIcon ||
                 cds->dwData == kCopyDataIconRect);
...
LRESULT TrayService::HandleCopyDataLocal(const COPYDATASTRUCT* cds) {
    ...
    if (cds->dwData == kCopyDataAppBar) {
        /* La gestione AppBar appartiene alla shell reale. Non restituiamo
         * successo senza eseguire un'operazione: il chiamante verra'
         * servito dal forwarding verso Explorer dopo questo ritorno. */
        return FALSE;
    }

    /* SHELLTRAYDATA reale (ManagedShell.Interop/NativeMethods.Shell32.cs):
     * { DWORD dwMessage; NOTIFYICONDATA nid; }. L'allineamento naturale
     * porta nid a offset 4 nei mittenti 32 bit e a offset 8 nei mittenti
     * 64 bit; non esiste un secondo DWORD semantico prima del messaggio.
     * Accettiamo entrambi i tracciati perche' un processo 32 bit puo'
    ...
    if (cds->dwData == kCopyDataTrayIcon &&
        ...
        for (const size_t candidate : nidCandidates) {
            ...
            memcpy(&candidateSize, bytes + candidate, sizeof(candidateSize));
            const size_t available = cds->cbData - candidate;
    ...
    if (cds->dwData == kCopyDataIconRect && cds->cbData >= 40) {
        uint32_t magic = 0;
