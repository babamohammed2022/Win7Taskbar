...
#define CHARMS_PW_EDIT_H           38  // field height
#define CHARMS_PW_EDIT_W            0  // field width; 0 = pane width minus margins
#define CHARMS_PW_FIELD_GAP        40  // field -> buttons gap
#define CHARMS_PW_LABEL_SIZE       18  // "Enter the network security key" size
...
#define CHARMS_EXP_BTN_W          100  // Connect / Cancel / Disconnect width
#define CHARMS_EXP_BTN_H           36  // ...and height
#define CHARMS_EXP_BTN_TEXT_SIZE   17  // button label size
...
#define CHARMS_RIGHT_SHIFT_TOGGLE   0  // both On/Off toggle switches
#define CHARMS_RIGHT_SHIFT_SIGNAL   0  // signal bars (+ the Connected label)
...
#define CHARMS_SLIDE_IN_MS       575
#define CHARMS_SLIDE_OUT_MS      600
...
#define CHARMS_TAIL_SNAP_PX      0.75f
...
void InitGlobalFonts() {
    ...
    // parameters (ANTIALIASED_QUALITY) so the sliding text never flickers;
    // the settled twins below use CLEARTYPE_QUALITY for the sharpest text
    // once the pane stops moving (this is what removes the "pixelly" look).
    ...
    g_hFontCharmsTitleAA   = CreateFontW(-ScaleDpi(CHARMS_FONT_TITLE_SIZE),0,0,0,CHARMS_FONT_TITLE_WEIGHT,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH|FF_DONTCARE,CHARMS_FONT_TITLE_FAMILY);
    g_hFontCharmsSectionAA = CreateFontW(-ScaleDpi(CHARMS_FONT_SECTION_SIZE),0,0,0,CHARMS_FONT_SECTION_WEIGHT,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH|FF_DONTCARE,CHARMS_FONT_SECTION_FAMILY);
    g_hFontCharmsNormalAA  = CreateFontW(-ScaleDpi(CHARMS_FONT_BODY_SIZE),0,0,0,CHARMS_FONT_BODY_WEIGHT,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH|FF_DONTCARE,CHARMS_FONT_BODY_FAMILY);
    g_hFontCharmsBoldAA    = CreateFontW(-ScaleDpi(CHARMS_FONT_BODY_SIZE),0,0,0,CHARMS_FONT_BODY_BOLD_WEIGHT,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH|FF_DONTCARE,CHARMS_FONT_BODY_FAMILY);
    ...
    g_hFontCharmsLinkAA = CreateFontW(-ScaleDpi(CHARMS_FONT_LINK_SIZE),0,0,0,CHARMS_FONT_LINK_WEIGHT,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH|FF_DONTCARE,CHARMS_FONT_LINK_FAMILY);
    ...
    g_hFontCharmsLightAA = CreateFontW(-ScaleDpi(CHARMS_FONT_BODY_SIZE),0,0,0,CHARMS_FONT_LIGHT_WEIGHT,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH|FF_DONTCARE,CHARMS_FONT_LIGHT_FAMILY);
    g_hFontCharmsConnectedAA = CreateFontW(-ScaleDpi(CHARMS_FONT_CONNECTED_SIZE),0,0,0,CHARMS_FONT_CONNECTED_WEIGHT,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH|FF_DONTCARE,CHARMS_FONT_CONNECTED_FAMILY);
    g_hFontCharmsConnectedWifiAA = CreateFontW(-ScaleDpi(CHARMS_FONT_CONNECTEDWIFI_SIZE),0,0,0,CHARMS_FONT_CONNECTEDWIFI_WEIGHT,0,0,0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH|FF_DONTCARE,CHARMS_FONT_CONNECTEDWIFI_FAMILY);
...
static void DrawCharmsStyleFlyout(HWND hwnd, HDC hdc, int panelW, int panelH) {
    ...
    if (showConnections) {
        ...
        SetTextColor(hdc, CHARMS_CONNECTED_COLOR);
    ...
    for (int i = 0; showWifiSections && i < paintState.networkCount; i++) {
        ...
        if (isExpanded) {
            ...
            if (needsPassword)
                blockH += ScaleDpi(CHARMS_PW_LABEL_GAP)
                        + ScaleDpi(CHARMS_PW_EDIT_H + CHARMS_PW_FIELD_GAP);
            else
        ...
        if (isExpanded) {
            ...
            if (needsPassword) {
                ...
                int editW = (CHARMS_PW_EDIT_W > 0)
                    ? ScaleDpi(CHARMS_PW_EDIT_W)
                    : (panelW - margin * 2);
            ...
            if (g_hWndCharmsCancelBtn) {
                if (needsPassword) {
                    MoveWindow(g_hWndCharmsCancelBtn,
                               btnX - btnW - ScaleDpi(8), curY, btnW, btnH, TRUE);
