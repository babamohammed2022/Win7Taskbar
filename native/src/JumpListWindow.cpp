...
int JumpListWindow::MeasureWidestRow() const {
    ...
    W7T_SEH_TRY {
        try {
            ...
            const int fontH = -::MulDiv(11, static_cast<int>(m_dpi), 96);
            /* SEMIBOLD deliberato: si misura il caso peggiore (alcune
             * righe usano la faccia piu' pesante) cosi' la stima non
             * puo' MAI tagliare. RAII su font/selezione/DC. */
            const UniqueGdiObject font(CreateFontW(fontH, 0, 0, 0,
            ...
            const SelectGuard select(hdc, font.get());
            for (const Row& r : m_rows) {
                const bool isDoc =
                    r.kind == Row::DocRecent ||
                    r.kind == Row::DocFrequent;
                    ...
                    : (r.kind == Row::App
                        ...
                        : (r.kind == Row::Close
                            ...
                            : (r.kind == Row::Pin
                                ...
                                : iconLeft)));
                RECT bounds{ 0, 0, 0, 0 };
                DrawTextW(hdc, r.label.c_str(), -1, &bounds,
                          DT_CALCRECT | DT_SINGLELINE | DT_NOPREFIX);
                const int need =
                    textLeft + (bounds.right - bounds.left) + Sc(12);
                if (need > best) {
                    best = need;
                }
            }
        } catch (...) {
            /* measuring failed: the base width survives */
        }
    } W7T_SEH_CATCH {
    } W7T_SEH_END
