...
Win7Taskbar is still under development. Some features are incomplete or recreated, particularly parts of the notification area and system UI on Windows 11. Window thumbnail previews use a direct DWM surface with the existing image border and close button, without a coloured backing panel. Jump Lists open the Windows 7 way: press a task button with the left button and drag away from the bar (up for a bottom bar) - the list appears above the button, follows the cursor and the row under it is highlighted; releasing on a row activates it, releasing over the list or the button leaves the list open to click, releasing elsewhere cancels. A plain click still activates the window and a drag along the bar still reorders the icons (the two gestures arbitrate by the first threshold crossed, so they never conflict). The small up-arrow the hovered button shows at its right edge is a secondary trigger (a left click on it opens the list directly). The list shows the application's real Shell data and never appears from the right-click, whose menu is unchanged.
