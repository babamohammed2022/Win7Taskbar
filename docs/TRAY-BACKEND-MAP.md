...
| `NotificationArea.IconDataCallback` answers `Shell_NotifyIconGetRect` (`dwData = 3`) with `MAKELPARAM(left, top)` for `dwMessage = 1` and `MAKELPARAM(right, bottom)` for `dwMessage = 2` | `TrayService::HandleCopyDataLocal` returns the same packed `LRESULT`; `HandleCopyData` passes Explorer's reply through when the icon is not ours | The previous reply was a plain `TRUE`, unusable by the caller; the packed form truncates coordinates to 16 bits like the reference | `HandleCopyData` (`isRectQuery` branch), `ForwardCopyDataToExplorer(..., responseOut)` |
