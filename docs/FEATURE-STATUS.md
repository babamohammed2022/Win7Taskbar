...
- **v2.62 (RE-verified content):** after reverse-engineering the Windows 7
  ...
  `docs/JUMPLIST-RE-VERIFICATION.md`), the list now carries the Windows 7
  sections: the **pinned (custom) items** - read with
  `IApplicationDestinations`, the documented read view of the store the
  Windows 7 taskbar reads directly
  (`HKCU\...\Explorer\ApplicationDestinations\<AppID>`, evidenced in the
  dump) - shown above the document sections with their real file icons;
  **Recent/Frequent** via `IApplicationDocumentLists` (unchanged); and a
  **Tasks** section (Minimize/Maximize/Restore/Move/Size, the pre-existing
  graphical close row stays) for groups with a live window, reusing the
  existing native window-command path. Item behavior follows the same
  evidence: a click opens the item (`customopen`), a right-click on a
  pinned row offers the unpin (`togglepin`) through the documented write
  API (`ICustomDestinationList`), and the list refreshes in place.
  ...
  `NoJumpListPathTooltip` case). An earlier code comment claiming the
  pinned list cannot be read through the public API was wrong and has
  been corrected.
